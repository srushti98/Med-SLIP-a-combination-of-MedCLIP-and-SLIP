...
def valid_epoch(model, valid_loader, criterion, metric,accuracy_metric,precision_metric,recall_metric,f1_score_metric,mode='val'):
    ...
    return loss_meter, {'Validation AUROC': val_metric_result.item(), 'val acc': acc.item(), 'val pre': prec.item(), 'val f1 score': f1_score.item(), 'val recall': recall.item(), 'loss': loss_meter.avg}
...
# Load the saved state dictionary
    ...
    image_encoder=torch.load(args.ipath)
    ...
    for epoch in range(args.epochs):
       ...
       # print("len of p_non_wd:", len(p_non_wd))
        train_loss, train_report = train_epoch(model, train_loader, optimizer, criterion, lr_scheduler, train_metric,accuracy_metric,precision_metric,recall_metric,f1_score_metric)
        model.eval()
        with torch.no_grad():
            valid_loss, validation_report = valid_epoch(model, valid_loader, criterion, val_metric,accuracy_metric,precision_metric,recall_metric,f1_score_metric)
    ...
    model.eval()
    with torch.no_grad():
        test_loss,test_report = valid_epoch(model, test_loader,criterion,test_metric,accuracy_metric,precision_metric,recall_metric,f1_score_metric,mode='test')
